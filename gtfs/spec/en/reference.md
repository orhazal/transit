...
For a given ordered pair of arriving trip and departing trip, the transfer with the greatest specificity that applies between these two trips is chosen. A transfer that is not in effect on the service day of the arriving trip (see `service_id`) is ignored, and the transfer with the next greatest specificity that applies is chosen instead. For any pair of trips, there should not be two transfers with equally maximal specificity that could apply.
...
|  `min_transfer_time` | Non-negative integer | **Conditionally Required** | Amount of time, in seconds, that must be available to permit a transfer between routes at the specified stops. The `min_transfer_time` should be sufficient to permit a typical rider to move between the two stops, including buffer time to allow for schedule variance on each route. <br><br>Conditionally Required:<br>- **Required** if `transfer_type` is `2`. <br>- Optional otherwise. |
|  `service_id` | Foreign ID referencing `calendar.service_id` or `calendar_dates.service_id` | Optional | Identifies a set of service days on which the transfer rule is in effect. The rule is in effect only when the service day of the arriving trip is included in the set of days referenced by `service_id`. If empty, the rule is in effect on all days.<br><br>`service_id` restricts the days on which an existing rule is in effect, and does not add specificity: it is not part of the primary key, therefore two records must not differ only by `service_id`. On service days where the rule is not in effect, it is ignored when selecting the transfer, as described in the ranking of specificity above. |
